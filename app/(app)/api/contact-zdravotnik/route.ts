...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      to: ['pata@bombovo.sk'],
      subject: 'Nova prihlaska Zdravotnik',
      html: `<!DOCTYPE html><html><body style="font-family:sans-serif;color:#222;padding:20px">
<h2 style="color:#1a1a2e">Nová prihláška – Zdravotník</h2>
<table style="border-collapse:collapse;width:100%;max-width:600px">
  ...
  ${row('Ukončené zdravotné vzdelanie', v(body.vzdelanie))}
  ${row('Prax', v(body.prax))}
  ...
  ${row('Termín školenia', v(body.termin))}
    ...
    if (error) {
      console.error('[contact-zdravotnik] Resend error:', error)
      return NextResponse.json({ error: 'Nastala chyba pri odosielaní' }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('[contact-zdravotnik] Error:', err)
