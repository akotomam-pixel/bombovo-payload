...
export default function RegistrationPage() {
  ...
  // Find camp and date by registrationId
  let selectedCamp: any = null;
  let selectedDate: any = null;
  let campDetails: any = null;

  for (const camp of allCamps) {
    const details = getCampDetails(camp.id);
    if (details) {
      const date = details.section5.dates.find(
        (d: any) => d.registrationId === registrationId,
      );
      if (date) {
        selectedCamp = camp;
        selectedDate = date;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // TODO: Handle form submission (send to API, email, etc.)
    console.log("Form submitted:", formData);
    setIsSubmitted(true);
  };

  if (!selectedCamp || !selectedDate || !campDetails) {
    return (
  ...
  return (
    <div className="min-h-screen flex flex-col bg-bombovo-gray">
      ...
      <main className="flex-grow py-8 md:py-12">
        {/* Header Section - FULL WIDTH, ONE LINE */}
        <div className="mb-8 px-4">
          {/* Image + Headline - in ONE line, centered, FULL WIDTH */}
          ...
          {/* Date and Location - in one line, centered, with yellow icons */}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="bg-white rounded-3xl border-4 border-bombovo-blue p-6 md:p-8">
              <div className="text-center py-16 space-y-6">
                ...
                <p className="text-base md:text-lg text-bombovo-dark leading-relaxed">
                  Ak máte nejaké ďalšie otázky, prosím neváhajte nás kontaktovať na email: <a href="mailto:bombovo@bombovo.sk" className="text-bombovo-red font-semibold hover:underline">bombovo@bombovo.sk</a> alebo na čísle <a href="tel:+421915774213" className="text-bombovo-red font-semibold hover:underline">+421 915 774 213</a>
                </p>
              </div>
            </div>
          ) : (
            /* Form - ONLY THIS HAS BLUE BORDER */
            <form
            ...
            >
              ...
              <div className="mb-8">
                ...
                <p className="text-sm text-gray-600 mt-2">
                  Pri voľbe "Nie" bude dieťa nosiť červený náramok a nebude na
                  spoločných fotkách.
                </p>
              </div>
              ...
              <div className="mb-8">
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  />
                  <span className="text-bombovo-dark font-semibold">
                    Súhlasím so spracovaním osobných údajov a so všetkými
                    podmienkami platnými na rok 2026 *
                  </span>
                </label>
              </div>
          ...
          {/* Price Summary - Only show when form is active */}
          {!isSubmitted && (
            <div className="mt-8 bg-white rounded-3xl border-4 border-bombovo-yellow p-6 md:p-8">
              <h2 className="text-2xl font-bold text-bombovo-dark mb-4">
                Cenový prehľad
              </h2>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-semibold text-bombovo-dark line-through">
                    {selectedDate.originalPrice}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-bombovo-dark font-bold">
                    Zľavnená cena:
                  </span>
                  <span className="text-2xl font-bold text-bombovo-red">
                    {selectedDate.discountedPrice}
                  </span>
                </div>
                {formData.insurance && (
